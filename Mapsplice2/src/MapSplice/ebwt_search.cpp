...
MUTEX_T gLock;
pthread_mutex_t sam_outfs_lock = PTHREAD_ADAPTIVE_MUTEX_INITIALIZER_NP;
pthread_mutex_t fusion_outfs_lock = PTHREAD_ADAPTIVE_MUTEX_INITIALIZER_NP;
map<string, string> reference_sequence;
map<string, size_t> chrom_table;
map<string, vector<Cluster> > fusion_cluster_vec;
...
vector<ofstream*> mapsplice_output_fs;
vector< pair<ofstream*, ofstream*> > mapsplice_unmapped_fs;
ofstream mapsplice_fusion_fs;
vector<Mapping_Stats*> mapping_stats;
...
int new_seg_thresh;
int anchor_length;
size_t max_double_splice_mismatch;
size_t max_single_splice_mismatch;
size_t max_append_mismatch;
int min_mapped_len;
int min_intron;
int max_intron_double_anchor;
int max_intron_single_anchor;
int extension_length;
int max_insertion;
int max_deletion;
bool do_pairing;
int max_pair_dist;
int fusion_extension;
bool load_juncdb;
bool load_fusiondb;
bool mapsplice_debug ;
...
int cluster_support_threshold;
int cluster_length_threshold;
int total_num_read_detected;
int min_read_len_detected;

string chrom_table_file;
string reference_seq_path;
string juncdb_index_file;
string fusiondb_index_file;
string output_filename;
string output_fusion_filename;
string output_unmapped_filename;
string fusion_cluster_filename;
string check_read_format_log;
...
static void resetOptions() {
  ...
  single_anchor_noncanonical = false;	
	...
	max_deletion = 10;
	fusion = false;
	fusion_double_anchor_noncanonical = false;
	fusion_single_anchor_noncanonical = false;
	new_seg_thresh = 15;
	anchor_length = 10;
	max_double_splice_mismatch = 2;
	max_single_splice_mismatch = 1;
	max_append_mismatch = 2;
	min_mapped_len = 0;
	min_intron = 50;
	max_intron_double_anchor = 200000;
	max_intron_single_anchor = 200000;
	extension_length = 3;
	do_pairing = true;
	max_pair_dist = 50000;
	fusion_extension = 10000;
	load_juncdb = false;
	load_fusiondb = false;
	mapsplice_debug = false;
	optimize_for_repeats = false;
	output_unmapped = false;
	output_unmapped_pe = false;
	splice_only = false;
	max_anchor_hits = 100;
	max_remap_hits = 100;  
	max_repeat_hits = 200;
	max_repeat_all_hits = 2000;
	maximum_alignments = 2000;
	do_local_align = false;
	local_align_max_mis = 2;
	local_align_max_gap = 4;
	local_align_max_dist = 10000;
	quality_scale = "phred64";
	mate_distance_training_set = 50000;
	min_fusion_distance = 10000;
	cluster_support_threshold = 2;
	cluster_length_threshold = 0;
	min_read_len_detected = 0;
	total_num_read_detected = 0;
}
...
enum {
	...
	ARG_MAPSPLICE_OUT,
	ARG_MIN_INTRON,
	ARG_MAX_INTRON_DOUBLE,
	ARG_MAX_INTRON_SINGLE,
	ARG_ANCHOR_LEN,
	ARG_DOU_SPLICE_MIS,
	ARG_SIN_SPLICE_MIS,
	ARG_APPEND_MIS,
	...
	ARG_FUSIONDB_INDEX,
	ARG_LOCAL_ALIGN,
	ARG_FUSION_DOUBLE_NONCANONICAL,
	ARG_FUSION_SINGLE_NONCANONICAL,
	ARG_QUAL_SCALE,
	ARG_CHECK_READ
};
...
static struct option long_options[] = {
	...
	{(char*)"min_len",            	   required_argument,       0,            ARG_MIN_LEN},
	{(char*)"seg_len",            	   required_argument,       0,            ARG_SEG_LEN},
	{(char*)"new_seg_thresh",     	   required_argument,       0,            ARG_NEW_SEG},	
	{(char*)"anchor_len",         	   required_argument,       0,            ARG_ANCHOR_LEN},	
	{(char*)"max_double_splice_mis",   required_argument,       0,            ARG_DOU_SPLICE_MIS},	
	{(char*)"max_single_splice_mis",   required_argument,       0,            ARG_SIN_SPLICE_MIS},	
	{(char*)"max_append_mis",     	   required_argument,       0,            ARG_APPEND_MIS},
	{(char*)"min_map_len",    	   	   required_argument,       0,            ARG_MIN_MAPPED},
	{(char*)"min_intron",         	   required_argument,       0,            ARG_MIN_INTRON},
	{(char*)"max_intron_double",       required_argument,       0,            ARG_MAX_INTRON_DOUBLE},
	{(char*)"max_intron_single",       required_argument,       0,            ARG_MAX_INTRON_SINGLE},
	{(char*)"max_ins",           	     required_argument,       0,            ARG_MAX_INS},	
	{(char*)"max_del",           	     required_argument,       0,            ARG_MAX_DEL},
	{(char*)"no_pairing",        	     no_argument,             0,            ARG_NO_PAIRING},
	{(char*)"splice_only",        	   no_argument,             0,            ARG_SPLICE_ONLY},
	{(char*)"max_pair_dis",       	   required_argument,       0,            ARG_MAX_PAIR_DIST},
	{(char*)"mapsplice_debug",    	   no_argument,             0,            ARG_MAPSPLICE_DEBUG},
	{(char*)"optimize_repeats",   	   no_argument,             0,            ARG_OPT_REAPEAT},
	{(char*)"local_align",   	         no_argument,             0,            ARG_LOCAL_ALIGN},
	{(char*)"output_unmapped",         required_argument,       0,            ARG_OUT_UNMAP},
	{(char*)"output_unmapped_pe",    	 required_argument,       0,            ARG_OUT_UNMAP_PE},
	{(char*)"max_anchor_hits",     	   required_argument,       0,            ARG_ANCHOR_HIT},
	{(char*)"max_remap_hits",      	   required_argument,       0,            ARG_REMAP_HIT},
	{(char*)"max_repeat_hits",     	   required_argument,       0,            ARG_REPEAT_HIT},
	{(char*)"max_repeat_all_hits", 	   required_argument,       0,            ARG_REPEAT_ALL_HIT},
	{(char*)"max_alignments", 	       required_argument,       0,            ARG_MAX_ALIGNMENT},
	{(char*)"chrom_tab",               required_argument,       0,            ARG_CHROM_TAB},
	{(char*)"ref_seq_path",            required_argument,       0,            ARG_REF_SEQ},
	{(char*)"juncdb_index",            required_argument,       0,            ARG_JUNCDB_INDEX},
	{(char*)"mapsplice_out",           required_argument,       0,            ARG_MAPSPLICE_OUT},
	{(char*)"qual-scale",          		 required_argument,       0,            ARG_QUAL_SCALE},
	{(char*)"double_anchor_noncanon",  no_argument,             0,            ARG_DOUBLE_NONCANON},
	{(char*)"single_anchor_noncanon",  no_argument,             0,            ARG_SINGLE_NONCANON},
	{(char*)"fusion_double_anchor_noncanon",  no_argument,      0,            ARG_FUSION_DOUBLE_NONCANONICAL},
	{(char*)"fusion_single_anchor_noncanon",  no_argument,      0,            ARG_FUSION_SINGLE_NONCANONICAL},
	{(char*)"fusion",                  required_argument,       0,            ARG_FUSION},
	{(char*)"cluster",                 required_argument,       0,            ARG_CLUSTER},
	{(char*)"fusiondb_index",          required_argument,       0,            ARG_FUSIONDB_INDEX},
	{(char*)"check_read",              required_argument,       0,            ARG_CHECK_READ},
...
static void parseOptions(int argc, const char **argv) {
	...
	do {
		...
		switch (next_option) {
			...
			case ARG_ORIG:
				...
				origString = optarg;
				break;
			case ARG_MIN_LEN: minimum_read_length = parseInt(18, "--min_len shortest read that will be aligned must be at least 18");  break;	
			case ARG_SEG_LEN: segment_length = parseInt(18, "--seg_len segment length must be at least 18");  break;	
			case ARG_NEW_SEG: new_seg_thresh = parseInt(12, "--new_seg_thresh new segment threshold must be at least 12");  break;
			case ARG_ANCHOR_LEN: anchor_length = parseInt(8, "--anchor_len anchor length must be at least 8"); break;
			case ARG_DOU_SPLICE_MIS: max_double_splice_mismatch = (size_t)parseInt(0, 2, "--max_double_splice_mis maximum double anchor splice mismatch must be between 0 and 2"); break;
			case ARG_SIN_SPLICE_MIS: max_single_splice_mismatch = (size_t)parseInt(0, 2, "--max_single_splice_mis maximum single anchor splice mismatch must be between 0 and 2"); break;
			case ARG_APPEND_MIS: max_append_mismatch = (size_t)parseInt(0, 3, "--max_append_mis maximum append mismatch must be between 0 and 3"); break;
			case ARG_MAX_INS: max_insertion = parseInt(0, 10, "--max_ins maximum insertion must be between 0 and 10"); break;
			case ARG_MAX_DEL: max_deletion = parseInt(0, 20, "--max_del maximum deletion must be between 0 and 20"); break;
			case ARG_MIN_INTRON: min_intron = parseInt(max_deletion + 1, "--min_intron minimum intron must be larger than max deletion"); break;
			case ARG_MAX_INTRON_DOUBLE: max_intron_double_anchor = parseInt(min_intron + 1, "--max_intron_double maximum intron must be larger than minimum intron"); break;
			case ARG_MAX_INTRON_SINGLE: max_intron_single_anchor = parseInt(min_intron + 1, "--max_intron_single maximum intron must be larger than minimum intron"); break;
			case ARG_OPT_REAPEAT: optimize_for_repeats = true; break;
			case ARG_LOCAL_ALIGN: do_local_align = true; break;
			case ARG_OUT_UNMAP: {output_unmapped = true; output_unmapped_filename = optarg; break;}
			case ARG_OUT_UNMAP_PE: {output_unmapped = true; output_unmapped_pe = true; output_unmapped_filename = optarg; break;}	
			case ARG_NO_PAIRING: do_pairing = false; break;
			case ARG_SPLICE_ONLY: splice_only = true; break;
			case ARG_MAX_PAIR_DIST: max_pair_dist = parseInt(1, "--max_pair_dist maximum pairing distance must be at least 1"); break;
			case ARG_MAPSPLICE_DEBUG: mapsplice_debug = true; break;
			case ARG_MIN_MAPPED: min_mapped_len = parseInt(0, "--min_map_len minimum mapped read length must be at least 0"); break;
			case ARG_CHROM_TAB: chrom_table_file = optarg; break;
			case ARG_REF_SEQ: reference_seq_path = optarg; break;
			case ARG_JUNCDB_INDEX: juncdb_index_file = optarg; load_juncdb = true; break;
			case ARG_MAPSPLICE_OUT: output_filename = optarg; break;
			case ARG_ANCHOR_HIT: max_anchor_hits = (uint32_t)parseInt(1, "--max_anchor_hits maximum anchor hits must be at least 1"); break;
			case ARG_REMAP_HIT: max_remap_hits = (uint32_t)parseInt(1, "--max_remap_hits maximum remap hits must be at least 1"); break;
			case ARG_REPEAT_HIT: max_repeat_hits = (uint32_t)parseInt(1, "--max_repeat_hits maximum repeat segment hits must be at least 1"); break;
			case ARG_REPEAT_ALL_HIT: max_repeat_all_hits = (uint32_t)parseInt(1, "--max_repeat_all_hits maximum repeat read hits must be at least 1"); break;
			case ARG_MAX_ALIGNMENT: maximum_alignments = (size_t)parseInt(1, "--max_alignments maximum alignments per read must be at least 1"); break;
			case ARG_DOUBLE_NONCANON: double_anchor_noncanonical = true; break;
			case ARG_SINGLE_NONCANON: single_anchor_noncanonical = true; break;
			case ARG_FUSION: {output_fusion_filename = optarg; fusion = true; break;}
			case ARG_CLUSTER: fusion_cluster_filename = optarg; break;
			case ARG_FUSIONDB_INDEX: fusiondb_index_file = optarg; load_fusiondb = true; break;
			case ARG_FUSION_DOUBLE_NONCANONICAL: fusion_double_anchor_noncanonical = true; break;
			case ARG_FUSION_SINGLE_NONCANONICAL: fusion_single_anchor_noncanonical = true; break;
			case ARG_CHECK_READ: check_read_format_log = optarg; break;
			case ARG_QUAL_SCALE: 
			{
				quality_scale = optarg;
				/*if(quality_scale == "phred64")
					phred64Quals = true;
				else if(quality_scale == "phred33")
					{solexaQuals = false; phred64Quals = false;}
				else if(quality_scale == "solexa64")
					solexaQuals = true;
				else
				{
					cerr << "Error: " << " quality scale type not recognized"  << endl;
					throw 1;
				}*/
				break;	
			}
	...
	if(segment_length == 0) {
		cerr << "Error: " << " segment length not specified"  << endl;
		throw 1;
	}
	if(chrom_table_file.empty()) {
		cerr << "Error: " << " chrom size file not specified"  << endl;
		throw 1;
	}
	if(reference_seq_path.empty()) {
		cerr << "Error: " << " reference sequence path not specified"  << endl;
		throw 1;
	}
	if(output_filename.empty()) {
		cerr << "Error: " << " mapsplice output filename not specified"  << endl;
		throw 1;
	}
	if(fusion && paired && fusion_cluster_filename.empty()){
		cerr << "Error: " << " fusion cluster not specified"  << endl;
		throw 1;
	}
...
static void* mismatchSearchWorkerFull(void *vp){
	...
	Read_Block read_block_pe1(paired);
	Read_Block read_block_pe2(paired);
	ReadBuf tmp_readbufa;
	ReadBuf tmp_readbufb;
	bool pe1_no_repeats = true;
	bool pe2_no_repeats = true;
	vector<bool> pe1_segment_no_repeats;
	vector<bool> pe2_segment_no_repeats;
	ofstream* unmapped_fs_ptr1 = NULL;
	ofstream* unmapped_fs_ptr2 = NULL;
	if(output_unmapped)
	{
		unmapped_fs_ptr1 = mapsplice_unmapped_fs[tid].first;
		unmapped_fs_ptr2 = mapsplice_unmapped_fs[tid].second;
	}
	MapSplice mapsplice(segment_length, new_seg_thresh, min_intron, max_intron_double_anchor, max_intron_single_anchor, extension_length, max_insertion, max_deletion, (format == FASTA), paired, seed, max_double_splice_mismatch, max_single_splice_mismatch, max_append_mismatch, min_mapped_len, anchor_length, load_juncdb, load_fusiondb, mapsplice_debug, mhits, max_anchor_hits, max_remap_hits, max_repeat_hits, max_repeat_all_hits, maximum_alignments, do_pairing, splice_only, max_pair_dist, optimize_for_repeats, output_unmapped, output_unmapped_pe, fusion, double_anchor_noncanonical, single_anchor_noncanonical, fusion_double_anchor_noncanonical, fusion_single_anchor_noncanonical, do_local_align, local_align_max_mis, local_align_max_gap, local_align_max_dist, quality_scale, mate_distance_training_set, min_fusion_distance);
	mapsplice.set_parameter(&bwt_vector, &read_block_pe1, &read_block_pe2, &chrom_table, &bt, patsrc, sink, mismatchSearch_ebwtFw, mismatchSearch_ebwtBw, juncdb_ebwt, juncdb_ebwtBw, fusiondb_ebwt, fusiondb_ebwtBw, &tmp_readbufa, &tmp_readbufb, &params, &reference_sequence, mapsplice_output_fs[tid], unmapped_fs_ptr1, unmapped_fs_ptr2, &sam_outfs_lock, &pe1_no_repeats, &pe2_no_repeats, &pe1_segment_no_repeats, &pe2_segment_no_repeats, &fusion_cluster_vec, &mapsplice_fusion_fs, &fusion_outfs_lock, mapping_stats[tid]);
	if(mapsplice_debug)
	{
		if(format == FASTQ)
			cout << "read format is fastq" << endl;
		else if(format == FASTA)
			cout << "read format is fasta" << endl;
		else
			cout << "read format is " << format << endl;
	}
	while(true) {
		sink->set_max_hits(mhits);
		bwt_vector.clear();
		read_block_pe1.clear();
		read_block_pe2.clear();
		pe1_no_repeats = true;
		pe2_no_repeats = true;
		pe1_segment_no_repeats.clear();
		pe2_segment_no_repeats.clear();
//cout << "getting read" << endl;
		GET_READ_ONLY(patsrc);
//cout << "getting read" << endl;
		// map pairend 1 segments
		patsrc->bufa().assign_readid_to_read_block(read_block_pe1);
		tmp_readbufa.copy(patsrc->bufa());
		int read_pe1_length = patsrc->bufa().length();
//cout << "read length is " << read_pe1_length << " minimum length is " << minimum_read_lengh << endl;
		int read_pe1_segment_no = 0;
		if(mapsplice_debug)
			cout << "mapping segment for " << read_block_pe1.read_id << ", read_length is " << read_pe1_length << endl;
		for(int segment_start = 0; segment_start < read_pe1_length;)
		{
			read_pe1_segment_no ++;
			int segment_end = 0;
			int read_left_len = read_pe1_length - segment_start;
			if(read_left_len < segment_length + new_seg_thresh)
				segment_end = read_pe1_length - 1;
			else
				segment_end = segment_start + segment_length - 1;
			patsrc->set_readbufa_segment(tmp_readbufa, segment_start, segment_end, seed, read_pe1_segment_no);
			patsrc->bufa().add_to_read_block(read_block_pe1);
			if(read_pe1_length >= minimum_read_length)
			{
				SET_PARAMS(patsrc);
				...
				#undef DONEMASK_SET
				bool segment_no_repeats = sink->convert_segment(bwt_vector, paired, 1, read_pe1_segment_no, mapsplice_debug);
				pe1_no_repeats =  segment_no_repeats && pe1_no_repeats;
				pe1_segment_no_repeats.push_back(segment_no_repeats);
				sink->finishRead(*patsrc, mapsplice_debug, false);
			}
			segment_start = segment_end + 1;
		}
		read_block_pe1.set_seg_num();
		//map pairend 2 segment
		if(paired)
		{
			tmp_readbufb.copy(patsrc->bufb());
			patsrc->bufb().assign_readid_to_read_block(read_block_pe2);
			int read_pe2_length = patsrc->bufb().length();
			int read_pe2_segment_no = 0;
			if(mapsplice_debug)
					cout << "mapping segment for " << read_block_pe2.read_id << ", read_length is " << read_pe2_length << endl;
			for(int segment_start = 0; segment_start < read_pe2_length;)
			{
				read_pe2_segment_no ++;
				int segment_end = 0;
				int read_left_len = read_pe2_length - segment_start;
				if(read_left_len < segment_length + new_seg_thresh)
					segment_end = read_pe2_length - 1;
				else
					segment_end = segment_start + segment_length - 1;
				patsrc->set_readbufa_segment(tmp_readbufb, segment_start, segment_end, seed, read_pe2_segment_no);
				patsrc->bufa().add_to_read_block(read_block_pe2);
				if(read_pe2_length >= minimum_read_length)
				{
					SET_PARAMS(patsrc);
					uint32_t plen = length(patFw);
					uint32_t s = plen;
					...
					#undef DONEMASK_SET
					bool segment_no_repeats = sink->convert_segment(bwt_vector, paired, 2, read_pe2_segment_no, mapsplice_debug);
					pe2_no_repeats = segment_no_repeats && pe2_no_repeats;
					pe2_segment_no_repeats.push_back(segment_no_repeats);
					sink->finishRead(*patsrc, mapsplice_debug, false);
				}
				segment_start = segment_end + 1;
			}
			read_block_pe2.set_seg_num();
		}
		if(mapsplice_debug)
			cout << "mapping segment done" << endl;
		mapsplice.align();
	} // End read loop
  WORKER_EXIT();
}
...
void readchrom(const char* filename, string& longseq)
{
	...
	if (longfile.is_open())
	{
		...
		cout << "Unable to open sequence file: " << filename << endl;
}

void load_reference_sequence(string reference_directory)
{
	map<string, size_t>::iterator it;
	for(it = chrom_table.begin(); it != chrom_table.end(); it++)
	{
		string chromfile = reference_directory + it->first;
		chromfile.append(".fa");
		string chromseq;
		readchrom(chromfile.c_str(), chromseq);
		transform(chromseq.begin(), chromseq.end(), chromseq.begin(), (int(*)(int)) toupper);
		reference_sequence.insert(make_pair(it->first, chromseq));
	}
}

void load_check_read_format_log(string check_read_format_log)
{
	ifstream input_fs(check_read_format_log.c_str());
	string line;
	while(getline(input_fs, line))
	{
		if(line[0] == '#')
			continue;
		char cmd_tmp[1000], parameter_tmp[1000];
		sscanf(line.c_str(), "%s %s", cmd_tmp, parameter_tmp);
		cout << "command: " << cmd_tmp << " parameter: " << parameter_tmp << endl;
		string cmd = cmd_tmp;
		if(cmd == "num_read")
			total_num_read_detected = atoi(parameter_tmp);
		else if(cmd == "minlen")
			min_read_len_detected = atoi(parameter_tmp);
	}
	cout << "num_read: " << total_num_read_detected << endl;
	cout << "min_read_len_detected: " << min_read_len_detected << endl;
	input_fs.close();
}

void load_chrom_table(string chrom_size_file)
{
	string line;
	ifstream chrom_size_fs(chrom_size_file.c_str());
	if(!chrom_size_fs)
	{
		cout << "open chromosome size file error" << endl;
		exit(1);	
	}
	while(getline(chrom_size_fs, line))
	{
		if(line=="")
			continue;
		char chrom_name_tmp[1000];
		size_t chrom_bp_len;
		sscanf(line.c_str(), "%s\t%zd",chrom_name_tmp, &chrom_bp_len);
		string chrom_name = chrom_name_tmp;
		chrom_table.insert(make_pair(chrom_name, chrom_bp_len));
	}
	chrom_size_fs.close();
}

void load_fusion_cluster(string cluster_file)
{
	string line;
	ifstream fusion_cluster_fs(cluster_file.c_str());
	if(!fusion_cluster_fs)
	{
		cout << "open fusion cluster file error" << endl;
		exit(1);	
	}
	int total_cluster = 0;
	if(total_num_read_detected != 0)
		cluster_support_threshold += (int)(log2(double(total_num_read_detected/20000000)));
	cout << "cluster_support_threshold: " << cluster_support_threshold << endl;
	cluster_length_threshold = max(minimum_read_length, min_mapped_len);
	if(min_read_len_detected)
		cluster_length_threshold = max(cluster_length_threshold, min_read_len_detected);
	while(getline(fusion_cluster_fs, line))
	{
		if(line=="")
			continue;
		char chrom1_tmp[1000], chrom2_tmp[1000];
		int strand1_tmp, strand2_tmp, start1, end1, start2, end2, support;
		sscanf(line.c_str(), "%s\t%d\t%d\t%d\t%s\t%d\t%d\t%d\t%d",chrom1_tmp, &strand1_tmp, &start1, &end1, chrom2_tmp, &strand2_tmp, &start2, &end2, &support);
		string strand1 = (strand1_tmp == 0 ? "+" : "-");
		string strand2 = (strand2_tmp == 0 ? "+" : "-");	
		string chrom1 = chrom1_tmp;
		string chrom2 = chrom2_tmp;	
		bool same_strand = (strand1 == strand2);	
		if(((end1 - start1 + 1) <= cluster_length_threshold && (end2 - start2 + 1) <= cluster_length_threshold )|| support < cluster_support_threshold)
			continue;
		Cluster new_fusion_cluster1(chrom1, strand1, max(0, start1 - fusion_extension), min((int)(chrom_table[chrom1]) - 1, end1 + fusion_extension), chrom2, strand2, max(0, start2 - fusion_extension), min((int)(chrom_table[chrom2]) - 1, end2 + fusion_extension), support, same_strand);
		Cluster new_fusion_cluster2(chrom2, strand2, max(0, start2 - fusion_extension), min((int)(chrom_table[chrom2]) - 1, end2 + fusion_extension), chrom1, strand1, max(0, start1 - fusion_extension), min((int)(chrom_table[chrom1]) - 1, end1 + fusion_extension), support, same_strand);
		fusion_cluster_vec[chrom1].push_back(new_fusion_cluster1);
		fusion_cluster_vec[chrom2].push_back(new_fusion_cluster2);
		total_cluster ++;
	}
	fusion_cluster_fs.close();
	for(map<string, vector<Cluster> >::iterator it = fusion_cluster_vec.begin(); it != fusion_cluster_vec.end(); it++)
		sort(it->second.begin(), it->second.end(), fusion_cluster_cmp);
}

void load_juncdb_index(string juncdb_index_path)
{
	string ebwt_file_check = juncdb_index_path + ".1.ebwt";
	if(access(ebwt_file_check.c_str(), 0) == 0)
	{
		juncdb_ebwt = new Ebwt<String<Dna, Alloc<> > >(juncdb_index_path,
	                ...
	                false /*passMemExc*/,
	                sanityCheck);
		juncdb_ebwt->loadIntoMemory(color ? 1 : 0, -1, !noRefNames, startVerbose);   
		//juncdb_ebwt->closeIndexFile();     
		juncdb_ebwtBw = new Ebwt<String<Dna, Alloc<> > >(juncdb_index_path + ".rev",
										color,  // index is colorspace
										-1,     // don't care about entireReverse
										...
										false /*passMemExc*/,
										sanityCheck);
		juncdb_ebwtBw->loadIntoMemory(color ? 1 : 0, -1, !noRefNames, startVerbose);  				
		//juncdb_ebwtBw->closeIndexFile();  
	}
	else
	{
		cout << "open juncdb index file error" << endl;
		exit(1);	
	}
}

void load_fusiondb_index(string fusiondb_index_path)
{
	string ebwt_file_check = fusiondb_index_path + ".1.ebwt";
	if(mapsplice_debug)
		cout << "loading fusion db index " << fusiondb_index_path << endl;
	if(access(ebwt_file_check.c_str(), 0) == 0)
	{
		fusiondb_ebwt = new Ebwt<String<Dna, Alloc<> > >(fusiondb_index_path,
	                color,  // index is colorspace
	                -1,     // don't care about entireReverse
	                true,     // index is for the forward direction
	                /* overriding: */ offRate,
	                ...
	                false /*passMemExc*/,
	                sanityCheck);
		fusiondb_ebwt->loadIntoMemory(color ? 1 : 0, -1, !noRefNames, startVerbose);    
		fusiondb_ebwtBw = new Ebwt<String<Dna, Alloc<> > >(fusiondb_index_path + ".rev",
										color,  // index is colorspace
										-1,     // don't care about entireReverse
										false, // index is for the reverse direction
										/* overriding: */ offRate,
										...
										false /*passMemExc*/,
										sanityCheck);
		fusiondb_ebwtBw->loadIntoMemory(color ? 1 : 0, -1, !noRefNames, startVerbose);
		if(mapsplice_debug)
			cout << "loading fusion db index complete" << endl;			
	}
	else
	{
		cout << "open fusiondb index file error" << endl;
		exit(1);	
	}
}

/*void load_splited_ebwt(string split_index_path, bool load_Bw)
{
	cout << "loading split index" << endl;
	map<string, size_t>::iterator it;
	for(it = chrom_table.begin(); it != chrom_table.end(); it++)
	{
		if(splited_ebwt.find(it->first) == splited_ebwt.end())
		{
			vector< Ebwt<String<Dna, Alloc<> > >* > new_ebwt_vec;
			splited_ebwt.insert(make_pair(it->first, new_ebwt_vec));
		}
		if(load_Bw && splited_ebwtBw.find(it->first) == splited_ebwtBw.end())
		{
			vector< Ebwt<String<Dna, Alloc<> > >* > new_ebwtBw_vec;
			splited_ebwtBw.insert(make_pair(it->first, new_ebwtBw_vec));	
		}
		for(int i = 1; (i - 1) * split_index_size + split_index_right_ext < it->second; i++)    //
		{
			string EbwtFile = split_index_path + it->first;
			EbwtFile.append(".fa").append(".").append(int_to_str(i));
			string ebwt_file_check = EbwtFile + ".1.ebwt";
			Ebwt<String<Dna, Alloc<> > >* new_ebwt = NULL;
			Ebwt<String<Dna, Alloc<> > >* new_ebwtBw = NULL;
			if(access(ebwt_file_check.c_str(), 0) == 0)
			{
				new_ebwt = new Ebwt<String<Dna, Alloc<> > >(EbwtFile,
	                color,  // index is colorspace
	                -1,     // don't care about entireReverse
	                true,     // index is for the forward direction
	                offRate,
	                ...
	                false,
	                sanityCheck);
	    	new_ebwt->loadIntoMemory(color ? 1 : 0, -1, !noRefNames, startVerbose);   
	   	 	new_ebwt->closeIndexFile();     
				if(load_Bw)
				{
					//cout << "loading reverse index: " << EbwtFile << ".rev" << endl;
					new_ebwtBw = new Ebwt<String<Dna, Alloc<> > >(EbwtFile + ".rev",
										color,  // index is colorspace
										-1,     // don't care about entireReverse
										false, // index is for the reverse direction
										 offRate,
										...
										false,
										sanityCheck);
					new_ebwtBw->loadIntoMemory(color ? 1 : 0, -1, !noRefNames, startVerbose);  				
					new_ebwtBw->closeIndexFile();  				
				}
			}
			//if(new_ebwt == NULL)
				//cout << "forward split index " << EbwtFile << " missing" << endl;
			//if(new_ebwtBw == NULL)
				//cout << "reverse split index " << EbwtFile << " missing" << endl; 
			splited_ebwt[it->first].push_back(new_ebwt);
			splited_ebwtBw[it->first].push_back(new_ebwtBw);
		}		
	}
		cout << "split index loading  complete" << endl;		
}*/

/*void free_split_index()
{
	map<string, size_t>::iterator it;
	for(it = chrom_table.begin(); it != chrom_table.end(); it++)
	{
	 	if(splited_ebwt.find(it->first) != splited_ebwt.end())
	 	{
	 		for(size_t i = 0; i < splited_ebwt[it->first].size(); i++)
	 		{
	 			if(splited_ebwt[it->first][i] != NULL)
	 			{
	 				if(splited_ebwt[it->first][i]->isInMemory())
						splited_ebwt[it->first][i]->evictFromMemory();
					delete splited_ebwt[it->first][i];
				}
			}
			splited_ebwt[it->first].clear();
		}
		if(splited_ebwtBw.find(it->first) != splited_ebwtBw.end())
		{
			for(size_t i = 0; i < splited_ebwtBw[it->first].size(); i++)
	 		{
				if(splited_ebwtBw[it->first][i] != NULL)
				{
					if(splited_ebwtBw[it->first][i]->isInMemory())
						splited_ebwtBw[it->first][i]->evictFromMemory();
					delete splited_ebwtBw[it->first][i];
				}
			}
			splited_ebwtBw[it->first].clear();
		}
	}
}*/

void free_remap_index()
{
	if(juncdb_ebwt->isInMemory()) 
	{
		juncdb_ebwt->evictFromMemory();
	}
	delete juncdb_ebwt;
	delete juncdb_ebwtBw;
}

void free_fusion_remap_index()
{
	if(fusiondb_ebwt->isInMemory()) 
	{
		fusiondb_ebwt->evictFromMemory();
	}
	delete fusiondb_ebwt;
	delete fusiondb_ebwtBw;
}

void free_reference_sequence()
{
	reference_sequence.clear();
}
	
void output_align_stats()
{
		int total_read_processed = 0;
		int total_read_aligned = 0;
		int total_read_spliced = 0;
		int total_read_fusion = 0;
		for(int i = 0; i < nthreads; i++)
		{
			total_read_processed += mapping_stats[i]->read_processed;
			total_read_aligned += mapping_stats[i]->read_aligned;
			total_read_spliced += mapping_stats[i]->read_spliced;
			total_read_fusion +=  mapping_stats[i]->read_fusion;
		}
		cout <<  "# of reads processed: " << total_read_processed << endl;
		cout <<  "# of reads aligned: " << total_read_aligned << " (" << setiosflags(ios::fixed) << setprecision(2) << (double(total_read_aligned) / double(total_read_processed) * 100) << "%) " << endl;
		cout <<  "# of reads have normal splice alignments: " << total_read_spliced << " (" << setiosflags(ios::fixed) << setprecision(2) << (double(total_read_spliced) / double(total_read_processed) * 100) << "%) " << endl;
		cout <<  "# of reads have fusion alignments: " << total_read_fusion << " (" << setiosflags(ios::fixed) << setprecision(2) << (double(total_read_fusion) / double(total_read_processed) * 100) << "%) " << endl;
		//if(paired)
		//	cout <<  "# of reads can be paired: " << (total_read_paired * 2) << " (" << total_read_paired << " pairs)" << endl;
}
                   ...
                   const string& outfile)
{
	...
	paired = mates1.size() > 0 || mates12.size() > 0;
	...
	load_chrom_table(chrom_table_file);
	load_reference_sequence(reference_seq_path);
	if(!check_read_format_log.empty())
		load_check_read_format_log(check_read_format_log);
	if(load_juncdb)
		load_juncdb_index(juncdb_index_file);
	if(load_fusiondb)
		load_fusiondb_index(fusiondb_index_file);
	for(int i = 0; i < nthreads; i++)
	{
		ofstream* new_output_fs = NULL;
		if(output_fusion_filename.empty() || mapsplice_debug)
		{
			string mapsplice_output_file_threaded = output_filename + "." + int_to_str(i + 1);
			new_output_fs = new ofstream(mapsplice_output_file_threaded.c_str());
			if(!(*new_output_fs))
			{
				cout << "open alignment output file error: " << mapsplice_output_file_threaded << endl;
				exit(1);	
			}
		}
		mapsplice_output_fs.push_back(new_output_fs);
		Mapping_Stats* new_mapping_stats = new Mapping_Stats();
		mapping_stats.push_back(new_mapping_stats);
	}
	if(!output_fusion_filename.empty())
	{
			mapsplice_fusion_fs.open(output_fusion_filename.c_str());
			if(!mapsplice_fusion_fs)
			{
				cout << "open fusion output file error: " << output_fusion_filename << endl;
				exit(1);	
			}
			if(!fusion_cluster_filename.empty())
				load_fusion_cluster(fusion_cluster_filename);
			fusion = true;
	}
	if(output_unmapped)
	{
		for(int i = 0; i < nthreads; i++)
		{
			string mapsplice_unmapped_file1_threaded = output_unmapped_filename + ".1" + "." + int_to_str(i + 1);
			string mapsplice_unmapped_file2_threaded = output_unmapped_filename + ".2" + "." + int_to_str(i + 1);
			ofstream* new_unmapped_file1 = new ofstream(mapsplice_unmapped_file1_threaded.c_str());
			ofstream* new_unmapped_file2 = new ofstream(mapsplice_unmapped_file2_threaded.c_str());
			if(*new_unmapped_file1 && *new_unmapped_file2)
				mapsplice_unmapped_fs.push_back(make_pair(new_unmapped_file1, new_unmapped_file2));	
			else
			{
				cout << "open unmapped output file error " << mapsplice_unmapped_file1_threaded << endl;
				cout << "open unmapped output file error " << mapsplice_unmapped_file2_threaded << endl;
				exit(1);	
			}
		}
	}
	...
	}
	{
		...
		free_reference_sequence();
		if(load_juncdb)
			free_remap_index();
		if(load_fusiondb)
			free_fusion_remap_index();
		if(output_unmapped)
		{
			for(int i = 0; i < nthreads; i++) 
			{
				mapsplice_unmapped_fs[i].first->close();
				delete mapsplice_unmapped_fs[i].first;
				mapsplice_unmapped_fs[i].second->close();
				delete mapsplice_unmapped_fs[i].second;
			}		
		}
		for(int i = 0; i < nthreads; i++) 
		{
			if(mapsplice_output_fs[i])
			{
				mapsplice_output_fs[i]->close();
				delete mapsplice_output_fs[i];
			}
			delete mapping_stats[i];
		}
		/*if(!quiet) {
